...
A diferencia de Ripio, la API publica de Kraken (/0/public/OHLC) SI devuelve
velas historicas reales, asi que este bot calcula RSI/MACD igual que Binance
(sin necesidad de armar un historial de precios propio).
...
# Capital total asignado y reparto en partes iguales por par
CAPITAL_TOTAL = float(os.getenv("KRAKEN_CAPITAL_INICIAL", "0"))
CAPITAL_POR_PAR = {par: CAPITAL_TOTAL / len(PARES) for par in PARES}
...
def main_analysis(posiciones: dict, supabase, api_key: str, api_secret: str) -> None:
    """
    ...
    (RSI+MACD), igual que Bot Binance/main.py.
    """
    for par in PARES:
...
def main() -> None:
    ...
    balance_inicial = obtener_balance_kraken(api_key, api_secret, ACTIVO_KRAKEN)
    logger.info(f"Balance inicial disponible en Kraken: {balance_inicial:.2f} {MONEDA_CAPITAL}")
    ...
    enviar_telegram(
        ...
        f"Balance {MONEDA_CAPITAL}: {balance_inicial:.2f}\n"
    ...
    while True:
        try:
            ...
            main_analysis(posiciones, supabase, api_key, api_secret)
        except Exception as error:
